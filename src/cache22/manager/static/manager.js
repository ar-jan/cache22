...
(() => {
  ...
  function actions(getIds, after) {
    ...
    for (const [value, text] of [["check", "Check remote"], ["fetch", "Fetch"], ["unqueue", "Remove pending work"], ["schedule", "Set schedule"], ["disable", "Disable schedule"]]) {
    ...
    async function refresh() {
      ...
      if (queue) {
        ...
        $("#c22-jobs").replaceChildren(table([["Job", "id"], ["Repository", repositoryLink], ["Operation", "kind"], ["Origin", "origin"],
          ["State", "state"], ["Attempt", "attempt_number"], ["Due / retry", "due_at"], ["Elapsed (s)", "elapsed_seconds"], ["Phase", progress], ["Diagnostic", "error"]], data.jobs));
      } else {
